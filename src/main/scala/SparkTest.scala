...
object SparkTest extends App {
  ...
  //df.groupBy("transactionDay").sum("transactionAmount").orderBy("transactionDay").show(29)
  //df.groupBy("transactionDay","accountId","category").avg("transactionAmount").orderBy("transactionDay","accountId").show(100)

  //Question 2: Calculate the average value of transactions per account for each type of transaction
  //val newDF = df.groupBy("transactionDay","accountId").pivot("category").avg("transactionAmount")
  //newDF.na.fill(0.0).orderBy("transactionDay","accountId").show()

  //Question 3: Calculate statistics for each account number for the previous five days of transactions
  val newDF = df.groupBy("transactionDay", "accountId")
  ...
  Option(result.repartition(1).write.option("header", "true").csv("/Users/zhuj/scala-quantexa/out/result9"))
